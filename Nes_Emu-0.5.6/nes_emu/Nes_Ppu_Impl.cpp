...
Nes_Ppu_Impl::Nes_Ppu_Impl()
{
	...
	host_palette_size_ = 0;
	host_palette_max = palette_size;
	mapper = NULL;
	tile_cache_mem = NULL;
	caller_pixels = NULL;
	set_sprite_limit( 8 );
	set_palette_range( palette_size );
	set_pixels( NULL, 0 );
...
blargg_err_t Nes_Ppu_Impl::open_chr( byte const* new_chr, long chr_rom_size )
{
	...
	BLARGG_CHECK_ALLOC( tile_cache_mem );
...
{
	host_palette_size_ = 0;
	palette_offset = base_palette_offset;
...
{
	unsigned long new_offset = palette_offset;
	if ( host_palette_size_ > 0 )
		new_offset += palette_size * 0x01010101;
	if ( 0x100 - (palette_offset & 0xff) < palette_size )
		return;
	
	if ( host_palette_max - host_palette_size_ < palette_size )
		return;
	
	palette_offset = new_offset;
	
	byte* out = host_palette + host_palette_size_;
	host_palette_size_ += palette_size;
	
	memcpy( out, palette, 32 );
	int bg = palette [0];
	for ( int i = 0; i < 32; i += 4 )
		out [i] = bg;
	memcpy( out + 32, out, 32 );
}
