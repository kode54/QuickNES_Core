...
static const TCHAR * item_command[] = { _T( "Rewind (toggle)" ), _T( "Rewind (hold)" ), _T( "Rewind (analog hold)" ), _T( "Fast forward (toggle)" ), _T( "Fast forward (hold)" ), _T( "Fast forward (analog hold)" ), _T( "Toggle pad 1 rapid fire" ), _T( "Toggle pad 2 rapid fire" ), _T( "Pause emulation (toggle)" ), _T( "Pause emulation (hold)" ), _T( "Frame advance" ), _T( "Frame advance (hold to run)" ) };
...
private:
	...
	void add_to_list( HWND w, const dinput::di_event & e, unsigned action )
	{
		...
		if ( item != -1 )
		{
			...
			else if ( action >= 16 && action <= 27 ) str = item_command[ action - 16 ];
