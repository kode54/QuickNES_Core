...
static const TCHAR * item_command[] = { _T( "Rewind (toggle)" ), _T( "Rewind (hold)" ), _T( "Toggle pad 1 rapid fire" ), _T( "Toggle pad 2 rapid fire" ) };
...
private:
	...
	void add_to_list( HWND w, const dinput::di_event & e, unsigned action )
	{
		...
		if ( item != -1 )
		{
			...
			else if ( action >= 16 && action <= 19 ) str = item_command[ action - 16 ];
