...
{
	impl->brightness = (float) setup->brightness * (0.5f * rgb_unit2) + rgb_offset;
	impl->contrast   = (float) setup->contrast   * (0.5f * rgb_unit2) + rgb_unit2;
...
#include <limits.h>
	...
	#else
		#include <stdint.h>
		typedef uint16_t nes_ntsc_out_t;
	#endif
	...
	#else
		#include <stdint.h>
		typedef uint32_t nes_ntsc_out_t;
	#endif
