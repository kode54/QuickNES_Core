...
bool uxtheme_handle::g_create(uxtheme_api_ptr & p_out)
{
	bool rv = false;
	HINSTANCE inst = LoadLibrary(_T("uxtheme"));
	if (inst)
	{
		p_out = new uxtheme_handle(inst);
		rv = true;
	}
	return rv;
	}
	...
	int iPartId, int iStateId, const RECT *pRect, OPTIONAL const DTBGOPTS *pOptions)
{
	...
	if (!pDrawThemeBackgroundEx)
		pDrawThemeBackgroundEx = (DrawThemeBackgroundExProc)GetProcAddress(inst_uxtheme, "DrawThemeBackgroundExProc");
	if (pDrawThemeBackgroundEx)
