...
public:
	...
	enum { palette_size = 64 }; // number of entries used in host palette
...
// Additional optional features (can be ignored without any problem)
	...
	// Set first host palette entry to use for graphics rendering. Defaults to
	...
	void set_palette_start( int entry );
...
private:
	...
	enum { add_wait_states = false }; // to do: wait-states cause shaky graphics
	...
	nes_time_t cpu_time_() const;
	nes_time_t cpu_time() const { return add_wait_states ? cpu_time_() : cpu.time(); }
...
inline void Nes_Emu::set_palette_start( int i ) { ppu.set_palette_start( i ); }
