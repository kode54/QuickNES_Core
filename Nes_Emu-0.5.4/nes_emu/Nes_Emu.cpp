...
bool const add_wait_states2 = false;
...
nes_time_t Nes_Emu::cpu_time_() const
{
...
nes_time_t Nes_Emu::emulate_frame( unsigned long joypad1, unsigned long joypad2 )
{
	...
	while ( true )
	{
		...
		{
			// frame length might not be current
			sync_ppu();
		...
		if ( add_wait_states2 )
			next_event = min( next_event, impl->apu.next_dmc_read_time() );
