...
protected:
	...
	void write_2007( nes_time_t, int );
	void first_scanline();
...
inline void Nes_Ppu_Impl::write_2007( nes_time_t time, int data )
{
	int addr = access_2007( time );
	if ( addr < 0x2000 )
	{
	...
	{
		palette [map_palette( addr )] = data & 0x3f;
	}
}

inline void Nes_Ppu_Impl::set_palette_start( int entry )
{
	assert( entry % palette_size == 0 );
	assert( (unsigned) entry < 0x100 );
	palette_offset = entry * 0x01010101;
}
