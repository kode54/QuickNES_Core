...
struct vrc6_state_t
{
	...
	vrc6_sound_state_t sound_state;
};
BOOST_STATIC_ASSERT( sizeof (vrc6_state_t) == 26 + sizeof (vrc6_sound_state_t) );
...
public:
	Mapper_Vrc6( int sm ) :
			Nes_Mapper( "VRC6", STATIC_CAST(vrc6_state_t*) (this), sizeof (vrc6_state_t) ),
			swap_mask( sm )
	{
		nes_vrc6_ = &sound;
	}
...
//      dprintf( "Change mirroring %d\n", data );
	...
	void apply_mapping()
	{
		set_prg_bank( 0, bank_16k, prg_16k_bank );
		set_prg_bank( 2, bank_8k, prg_8k_bank );
...
//              dprintf( "%d timer expired\n", next_time );
	...
	void end_frame( nes_time_t end_time )
	{
		...
		sound.end_frame( emu()->apu_time( end_time ) );
	}
		...
		else switch ( addr & 0xf003 )
		{
			...
			default: {
				...
				if ( (unsigned) osc < sound.osc_count && reg < sound.reg_count )
					sound.write_osc( emu()->apu_time( time ), osc, reg, data );
				break;
			}
		}
	}
...
Nes_Mapper* Nes_Mapper::make_vrc6a()
{
	return BLARGG_NEW Mapper_Vrc6( 0 );
}

Nes_Mapper* Nes_Mapper::make_vrc6b()
{
